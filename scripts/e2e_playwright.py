    ...
    for name, viewport in {'desktop':{'width':1440,'height':1000},'tablet':{'width':900,'height':1000},'mobile':{'width':390,'height':844}}.items():
      ...
      def api(route):
        ...
        if request.url.endswith('/catalog/weather-artist'): route.fulfill(content_type='application/json',body=json.dumps(envelope(fixture['catalog'])))
          ...
          body=json.loads(request.post_data or '{}'); simulations.append(body); data={**fixture['simulation'],'patches':body['patches']}; route.fulfill(content_type='application/json',body=json.dumps(envelope(data)))
        else: route.continue_()
      page.route('**/api/v1/**', api)
      page.goto('http://127.0.0.1:5173',wait_until='networkidle'); page.evaluate('localStorage.clear()')
      page.get_by_label('캐릭터 이름').fill('봄날꽃씨'); page.get_by_role('button',name='불러오기').click(); page.wait_for_load_state('networkidle')
      page.get_by_text('1,000.13').first.wait_for(); page.get_by_role('checkbox',name='보석 사용').uncheck(); page.wait_for_timeout(350)
      ...
      page.get_by_role('checkbox',name='보석 사용').uncheck(); page.wait_for_timeout(350); page.reload(wait_until='networkidle'); assert not page.get_by_role('checkbox',name='보석 사용').is_checked()
      page.get_by_role('button',name='새로고침').click(); page.wait_for_load_state('networkidle'); assert any(x.get('forceRefresh') for x in loads); assert page.evaluate('document.documentElement.scrollWidth <= innerWidth + 1'); assert not errors, errors
