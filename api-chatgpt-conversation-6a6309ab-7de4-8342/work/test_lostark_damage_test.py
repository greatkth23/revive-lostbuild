...
class ParserTests(unittest.TestCase):
    ...
    def test_v272_uses_calculated_attack_after_profile_mismatch(self):
        result = dut.calculate(
            self.parsed,
            include_arkgrid=True,
        ...
        self.assertEqual(
            result["damage"]["skillBaseRaw"],
            dut.FIXED["skillCoefficient"] * attack["final"]
            + dut.FIXED["skillConstant"],
        )
    ...
    def test_gale_umbrella_skill_models_for_spring_flower_seed(self):
        ...
        cases = {
            "바람송곳": {
                ...
                "reflux": Decimal("0.60"),
                "allDamage": [Decimal("0.60"), Decimal("0.60"), Decimal("0.95")],
                "deduplicatedDamage": [
                    Decimal("0.60"), Decimal("0.60"), Decimal("0.95")
                ],
                "newOverV26": Decimal("1.60") * Decimal("1.95"),
            },
            "칼바람": {
                ...
                "reflux": Decimal("0.95"),
                "allDamage": [Decimal("0.45"), Decimal("0.95")],
                "deduplicatedDamage": [Decimal("0.45"), Decimal("0.95")],
                "newOverV26": Decimal("1.45"),
            },
            "몰아치기": {
                "coefficients": [
                    Decimal("9.72"),
                ...
                "reflux": Decimal("0.60"),
                "allDamage": [Decimal("0.60"), Decimal("0.948")],
                "deduplicatedDamage": [Decimal("0.60")],
                "newOverV26": Decimal("1.948"),
            },
            "회오리 걸음": {
                ...
                "criticalTripod": Decimal("0"),
                "reflux": Decimal("0.95"),
                "allDamage": [Decimal("0.95"), Decimal("1.05")],
                "deduplicatedDamage": [Decimal("0.95"), Decimal("1.05")],
                "newOverV26": Decimal("2.05"),
            },
        }
        for skill_name, expected in cases.items():
            with self.subTest(skill=skill_name):
                ...
                full_tripod_result = dut.calculate(
                    parsed,
                    include_arkgrid=True,
                    rule_version="current-v2.7.0",
                    skill_name=skill_name,
                )
                deduplicated_result = dut.calculate(
                    parsed,
                    include_arkgrid=True,
                    rule_version="current-v2.7.1",
                    skill_name=skill_name,
                ...
                self.assertEqual(
                    [
                        effect["percent"]
                        for effect in full_tripod_result["skillModel"][
                            "tripodDamageEffects"
                        ]
                    ],
                    expected["allDamage"],
                ...
                self.assertLess(
                    abs(
                        full_tripod_result["damage"]["nonCriticalRaw"]
                        - result["damage"]["nonCriticalRaw"]
                        * expected["newOverV26"]
                    ),
                    Decimal("1e-27"),
                ...
                self.assertEqual(
                    full_tripod_result["critical"]["skillTripodCriticalDamage"],
                    expected["criticalTripod"],
                )
                self.assertEqual(
                    [
                        effect["percent"]
                        for effect in deduplicated_result["skillModel"][
                            "tripodDamageEffects"
                        ]
                    ],
                    expected["deduplicatedDamage"],
                )
                if skill_name == "몰아치기":
                    self.assertEqual(
                        deduplicated_result["damage"]["nonCriticalRaw"],
                        result["damage"]["nonCriticalRaw"],
                    )
                    self.assertEqual(
                        [
                    ...
                    self.assertEqual(
                        deduplicated_result["damage"]["hits"][2][
                            "tripodSource"
                        ],
                        "공간베기",
                    )
    ...
    def test_audit_snapshot_regression(self):
        ...
        result = dut.calculate(parsed, rule_version="current-v2.7.1")
        self.assertEqual(
