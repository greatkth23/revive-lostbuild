...
function ResultCard({ skill, baseline, candidate }: { skill: Skill; baseline: Result | undefined; candidate: Result | undefined }) {
  ...
  return <article className="result-card">
    ...
    {expanded && baseline && candidate && <div id={`result-${skill.id}`} role="region" aria-label={`${skill.displayName} 상세 결과`} className="result-detail">
      ...
      <h4>적용 트라이포드</h4><div className="breakdown-columns">{([['기준', baseline], ['변경', candidate]] as const).map(([label, result]) => <div key={`tripods-${label}`}><strong>{label}</strong><ul>{result.checkpoints.selectedTripods.length ? result.checkpoints.selectedTripods.map((tripod) => <li key={`${tripod.skillName}-${tripod.name}`}>{tripod.name} · 피해 {percentDisplay(tripod.damagePercent)} · 치명 피해 {percentDisplay(tripod.criticalDamagePercent)}</li>) : <li>적용 없음</li>}</ul></div>)}</div>
